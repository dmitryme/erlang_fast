...
encode(MsgFields = [{FieldName1, Value} | _], #field{type = Type, presence = Presence, name = FieldName2, operator = #tail{}},
   Context = #context{pmap = PMap})
when (FieldName1 =/= FieldName2) orelse ((FieldName1 == FieldName2) andalso (Value == absent)) ->
   {encode_type(Type, null, is_nullable(Presence)), MsgFields, Context#context{pmap = <<PMap/bitstring, 0:1>>}};
...
when (FieldName1 =/= FieldName2) orelse ((FieldName1 == FieldName2) andalso (Value == absent)) ->
   {encode_type(decimal, null, is_nullable(Presence)), MsgFields, Context#context{pmap = <<PMap/bitstring, 0:1>>}};

encode(MsgFields = [{FieldName1, {Mantissa, Exponent}} | MsgFieldsRest],
   F = #field{name = FieldName2, presence = mandatory, operator = #decFieldOp{exponent = EOp, mantissa = MOp}}, Context) ->
   {EBin, [], Context1} = encode([{FieldName1, Exponent}], F#field{type = int32, operator = EOp}, Context),
   {MBin, [], Context2} = encode([{FieldName1, Mantissa}], F#field{type = int64, operator = MOp}, Context1),
   {<<EBin/bitstring, MBin/bitstring>>, MsgFieldsRest, Context2};
...
encode([{GroupName, MsgGroupFields} | MsgFieldsRest],
   ...
   {<<GroupPMap/bitstring, BinGroupFields/bitstring>>, MsgFieldsRest, Context#context{pmap = <<PMap/bitstring, 1:1>>}};
