...
decode(Length, Data, NeedPMap, Context = #fast_context{template = Template}) ->
   case NeedPMap of
      true ->
         ...
         ?debugFmt("PMap = ~p~n", [PMapRest]),
         {Msg, Context2, Data2} = erlang_fast_segment:decode_fields(Data1, Context1#fast_context{pmap = PMapRest}),
         {Msgs, Context3, Data3} = decode(Length, Data2, NeedPMap, Context2#fast_context{template = Template}),
         {[Msg | Msgs], Context3, Data3};
      false ->
         {Msg, Context1, Data1} = erlang_fast_segment:decode_fields(Data, Context),
         {Msgs, Context2, Data2} = decode(Length, Data1, NeedPMap, Context1#fast_context{template = Template}),
