...
-module(erlang_fast_decode_decimal).
...
decode_instruction(Data, {decimal, FieldName, _, _, Presence, #constant{value = InitialValue}},
...
decode_instruction(Data, {decimal, FieldName, _, _, optional, #constant{}},
   Context = #fast_context{pmap = <<0:1, PMapRest/bitstring>>}) ->
   {{FieldName, absent}, Context#fast_context{pmap = PMapRest}, Data};

decode_instruction(Data, {decimal, FieldName, _, _, optional, #default{value = InitialValue}},
    Context = #fast_context{pmap = <<0:1, PMapRest/bitstring>>}) ->
...
decode_instruction(Data, {decimal, FieldName, _, _, Presence, #default{value = _InitialValue}},
...
decode_instruction(Data, {decimal, FieldName, _, _, Presence, #copy{dictionary = Dict, key = Key}},
...
decode_instruction(Data, {decimal, FieldName, _, _, optional, #copy{dictionary = Dict, key = Key, value = InitialValue}},
...
decode_instruction(Data, {_, FieldName, _, _, Presence, #delta{dictionary = Dict, key = Key, value = InitialValue}},
...
decode_instruction(Data, {decimal, FieldName, _, _, Presence, {ExpOp, MantOp}}, Context) ->
   case erlang_fast_decode_number:decode_instruction(Data, #int32{name = FieldName, presence = Presence, operator =
            ExpOp}, Context) of
      R = {{FieldName, absent}, _, _} ->
         R;
      {{FieldName, Exponent}, Context1, Data1} ->
         {{FieldName, Mantissa}, Context2, Data2} = 
            erlang_fast_decode_number:decode_instruction(Data1, #int64{name = FieldName, presence = mandatory,
               operator = MantOp}, Context1),
         {{FieldName, {Mantissa, Exponent}}, Context2, Data2}
   end;

decode_instruction(Data, {decimal, FieldName, _, _, Presence, undef}, Context = #fast_context{logger = L}) ->
